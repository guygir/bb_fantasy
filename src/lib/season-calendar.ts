...
export function getSeasonStartMs(season: number): number {
...
export function resolveCurrentSeason(now = new Date()): number {
...
function envSeasonOverride(): number | null {
...
export function resolveCurrentSeasonFromEnv(now = new Date()): number {
 ...
 */
export function competitiveRosterStartMs(season: number): number {
  return getSeasonStartMs(season) + 8 * 86400000;
}

/** YYYY-MM-DD (UTC) of Sunday of W2. */
export function competitiveRosterStartDate(season: number): string {

...
import type { PromotionTierConfig } from "@/lib/promotions-tier";
...
export function PromotionsView({
  ...
  const band = tier.promotionBandSize;
    ...
    <div className="text-sm text-gray-600">
      ...
      <div className="mb-4 max-w-3xl">
        ...
        <p className="mt-3 text-gray-500">
          ...
          <span className="font-medium text-bb-text">{band}</span> rows are the current promotion band.
        </p>
      </div>
      ...
      {!error && entries.length === 0 && (
        <p className="text-gray-600">
          ...
          <code className="rounded bg-gray-100 px-1">npm run fetch-promotions</code> has run, the table will
          appear here.
        </p>
      )}
        ...
        <>
          ...
          <div className="overflow-x-auto rounded-lg border border-bb-border">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {entries.map((row) => {
                  const inPromotionBand = row.display_rank <= band;

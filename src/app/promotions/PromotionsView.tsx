...
import {
  assignPromotionBandStyles,
  type FinalsInfo,
  type LatestRankChange,
  type PlayoffStatus,
  type PromotionEntry,
  type PromotionNewsBlock,
} from "@/lib/promotions";
 ...
 * Striped-green = "to-be-promoted" band, expanded incrementally when same-league finals
 * pairs sit in the green+striped band (one becomes champ, freeing a slot).
 */
function buildRowStyles(
  ...
  const styles = assignPromotionBandStyles(entries, bandSize, finalsByLeague);
      ...
      <div className="mb-4 flex flex-col gap-6 lg:flex-row lg:items-start lg:gap-8">
        <div className="min-w-0 flex-1 max-w-3xl">
          ...
          {tierId === "league3" && numBotLeagues != null && (
            ...
            filled; when two teams in the green or striped band from the same league face each other in the
            finals, one will become champion (freeing a slot), so the next team in line is shown in{" "}
            <strong className="text-bb-text">striped green</strong> (to-be-promoted; counted incrementally);{" "}
